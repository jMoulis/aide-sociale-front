...
import ConfirmationDeleteContent from '@/app/[locale]/admin/my-app/components/Builder/ConfirmationDeleteContent';
import { PublishedDot } from '../PublishedDot';
import PageVersionForm from './PageVersionForm';
import { useTranslations } from 'next-intl';
import DisplayOptions from './LeftPanel/DisplayOptions';
  ...
  return (
    <header className='p-4 bg-white'>
      <div className='flex items-center'>
        ...
        />
        {pageVersion?.isDirty ? (
          <span>{t('labels.unsavedChanges')}</span>
        ) : null}
        <div className='flex items-center'>
          <SaveButton
            disabled={Boolean(loading)}
